...
const PlacementPrep = () => {
  ...
  const handleResourceClick = async (item, type) => {
    let targetUrl = '';
    
    // Priority 1: Check if item has a direct YouTube URL
    if (item.url && item.url.includes('youtube.com/watch?v=')) {
      targetUrl = item.url;
      window.open(targetUrl, '_blank', 'noopener,noreferrer');
      return;
    } 
    // Priority 2: Extract video ID if present in search_query
    else if (item.search_query) {
      const videoIdMatch = item.search_query.match(/[?&]v=([a-zA-Z0-9_-]{11})/);
      if (videoIdMatch) {
        targetUrl = `https://www.youtube.com/watch?v=${videoIdMatch[1]}`;
        window.open(targetUrl, '_blank', 'noopener,noreferrer');
        return;
      }
    }
    
    // Priority 3: Search YouTube and redirect to first video
    // Create search query from channel + title or search_query
    const searchTerm = item.search_query 
      ? (item.channel ? `${item.channel} ${item.search_query}` : item.search_query)
      : `${item.channel || ''} ${item.title}`.trim();
    
    // Use YouTube's search with autoplay - this will show results but user can click first video
    // For better UX, we redirect to search results page
